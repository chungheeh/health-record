...
export default async function MyPage() {
  ...
  return (
    <main className="min-h-screen bg-bg-primary">
      {/* 헤더 */}
      <header className="sticky top-0 z-50 bg-bg-primary border-b border-we-border px-4 h-14 flex items-center">
        <h1 className="font-semibold text-text-primary">마이</h1>
      </header>

      <div className="px-4 pt-6 pb-24 space-y-4">
        {/* 프로필 카드 */}
        <div className="bg-bg-secondary rounded-[16px] p-5 flex items-center gap-4">
          <div className="w-14 h-14 rounded-full bg-accent flex items-center justify-center">
            <span className="text-bg-primary text-xl font-bold">{avatarLetter}</span>
          </div>
          ...
          {profile?.goal && (
            <span className="text-xs bg-accent/15 text-accent px-2.5 py-1 rounded-full font-medium shrink-0">
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            <div key={label} className="bg-bg-secondary rounded-[14px] p-3 text-center">
        ...
        {profile && (
          <div className="bg-bg-secondary rounded-[16px] overflow-hidden">
            <div className="px-4 py-3 border-b border-we-border">
            ...
            {[
              ...
              <div key={label} className="flex items-center justify-between px-4 py-3 border-b border-we-border last:border-0">
        ...
        {profile?.target_calories && (
          <div className="bg-bg-secondary rounded-[16px] overflow-hidden">
            <div className="px-4 py-3 border-b border-we-border">
              <p className="text-sm font-semibold text-text-primary">영양 목표</p>
            </div>
            <div className="grid grid-cols-2 gap-px bg-we-border">
              {[
                { label: '목표 칼로리', value: profile.target_calories, unit: 'kcal', color: 'var(--accent)' },
                ...
                <div key={label} className="bg-bg-secondary px-4 py-3 text-center">
        ...
        <div className="bg-bg-secondary rounded-[16px] overflow-hidden">
          {[
            ...
            <Link
              key={href}
              href={href}
              className="flex items-center justify-between px-4 py-4 border-b border-we-border last:border-0 hover:bg-bg-tertiary transition-colors"
            >
        ...
        <form action="/auth/signout" method="post">
          <button
            type="submit"
            className="w-full py-3.5 bg-bg-secondary text-we-danger font-medium rounded-[16px] text-sm"
          >
            로그아웃
          </button>
        </form>
