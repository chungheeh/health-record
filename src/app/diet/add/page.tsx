  ...
  return (
    <main className="min-h-screen bg-[#0f0f0f] flex flex-col">
      ...
      <header className="sticky top-0 z-50 bg-[#0f0f0f] border-b border-[#2a2a2a] px-4 h-14 flex items-center gap-3">
        ...
        <span className="text-xs bg-[#242424] text-[#C8FF00] px-2 py-1 rounded-full">
          {mealType}
        </span>
        {/* 숨긴 사진 input */}
        <input
        ...
        {!editId && (
          <button
            ...
            className="ml-1 p-2 rounded-[10px] bg-[#1a1a1a] border border-[#2a2a2a] text-[#888888] active:bg-[#242424] transition-colors disabled:opacity-50"
            title="사진으로 분석"
          >
            {isAnalyzing
      ...
      <div className="flex-1 flex flex-col">
        ...
        {!selected && (
          <div className="px-4 pt-4">
            <div className="flex items-center gap-2 bg-[#1a1a1a] rounded-[14px] px-4 border border-[#2a2a2a] focus-within:border-[#C8FF00] transition-colors">
              <Search size={18} className="text-[#555555] shrink-0" />
              <input
                ref={inputRef}
                type="text"
                value={query}
                ...
                className="flex-1 bg-transparent py-4 text-sm text-[#f0f0f0] outline-none placeholder:text-[#555555]"
                autoFocus
              />
              {query ? (
                ...
                <span className="text-[10px] text-[#444]">검색</span>
              )}
            </div>
            ...
            {query.length > 0 && results.length === 0 && !isSearching && (
              <button
                ...
                className="mt-3 w-full flex items-center gap-3 bg-[#1a1a1a] border border-dashed border-[#2a2a2a] rounded-[12px] px-4 py-3.5"
              >
                <Plus size={16} className="text-[#C8FF00] shrink-0" />
                <div className="text-left">
                  <p className="text-sm text-[#f0f0f0]">&quot;{query}&quot; 직접 입력</p>
                  <p className="text-[10px] text-[#555555] mt-0.5">DB에 없는 음식 — 영양소 직접 입력</p>
                </div>
              </button>
            )}
          </div>
          ...
          <div className="flex-1 overflow-y-auto mt-2 pb-4">
            ...
            {!isSearching && results.length > 0 && (
              <div>
                ...
                {results.filter(r => r.type === 'db').length > 0 && (
                  <>
                    <p className="px-4 py-2 text-[10px] font-semibold text-[#555555] uppercase tracking-wider">
                      식품 DB
                    </p>
                    {results.filter(r => r.type === 'db').map((r, i) => {
                      ...
                      return (
                        <button
                          ...
                          className="w-full flex items-center justify-between px-4 py-3.5 border-b border-[#1e1e1e] hover:bg-[#1a1a1a] active:bg-[#1a1a1a] transition-colors text-left"
                        >
                          <div className="flex-1 min-w-0 mr-3">
                            <p className="text-sm text-[#f0f0f0] truncate">
                              {f.brand && <span className="text-[#C8FF00] mr-1">{f.brand}</span>}
                              {f.name}
                            </p>
                          ...
                          <div className="text-right shrink-0">
                            <p className="text-sm font-bold text-[#f0f0f0] tabular-nums">
                              {Math.round(f.calories_per_100g * (f.serving_size_g ?? 100) / 100)}
                              <span className="text-xs font-normal text-[#555555]">kcal</span>
                            </p>
                          ...
                          <ChevronRight size={14} className="text-[#333] ml-2 shrink-0" />
                        </button>
                      )
                    })}
                  </>
                ...
                {results.filter(r => r.type === 'history').length > 0 && (
                  <>
                    <p className="px-4 py-2 text-[10px] font-semibold text-[#555555] uppercase tracking-wider mt-2">
                      최근 기록
                    </p>
                    {results.filter(r => r.type === 'history').map((r, i) => {
                      ...
                      return (
                        ...
                        >
                          <div className="flex-1 min-w-0 mr-3">
                            <p className="text-sm text-[#f0f0f0] truncate">{h.food_name}</p>
                            <p className="text-[11px] text-[#555555] mt-0.5">최근 기록</p>
                          </div>
                          <div className="text-right shrink-0">
                            <p className="text-sm font-bold text-[#f0f0f0] tabular-nums">
                              {Math.round(h.calories_per_100g)}
                              <span className="text-xs font-normal text-[#555555]">kcal/100g</span>
                            </p>
                          </div>
                          <ChevronRight size={14} className="text-[#333] ml-2 shrink-0" />
                        </button>
            ...
            {isAnalyzing && (
              <div className="flex flex-col items-center py-12 gap-3">
                ...
                <p className="text-sm text-[#888888]">AI가 음식을 분석하고 있어요...</p>
                <p className="text-xs text-[#555555]">잠시만 기다려주세요</p>
              </div>
            )}

            {/* 분석 에러 */}
            {analyzeError && !isAnalyzing && (
              <div className="mx-4 mt-3 bg-[#FF4B4B]/10 border border-[#FF4B4B]/30 rounded-[12px] px-4 py-3">
                <p className="text-sm text-[#FF4B4B]">⚠️ {analyzeError}</p>
              </div>
            )}
            ...
            {!isSearching && !isAnalyzing && query.length === 0 && (
              <div className="px-4 py-8 text-center">
                ...
                <p className="text-sm text-[#555555]">음식 이름 또는 브랜드를 검색하세요</p>
                <p className="text-xs text-[#444] mt-1">예: 하림, 닭가슴살, 현미밥, 아메리카노...</p>
                <button
                  onClick={() => photoInputRef.current?.click()}
                  className="mt-5 flex items-center gap-2 mx-auto bg-[#1a1a1a] border border-[#2a2a2a] rounded-[12px] px-5 py-3 text-sm text-[#888888] active:bg-[#242424]"
                >
                  <Camera size={16} className="text-[#C8FF00]" />
                  사진으로 AI 분석하기
                </button>
              </div>
            )}
        ...
        {selected && (
          <div className="flex-1 px-4 pt-4 pb-32 space-y-4 overflow-y-auto">
            {/* 선택된 음식 헤더 */}
            <div className="bg-[#1a1a1a] rounded-[16px] p-4 flex items-center gap-3">
              <div className="flex-1 min-w-0">
                <p className="text-base font-bold text-[#f0f0f0] truncate">{selected.name}</p>
                <p className="text-xs text-[#555555] mt-0.5">100g당 {Math.round(selected.per100.cal)}kcal</p>
              </div>
              <button
                onClick={() => { setSelected(null); setQuery(''); setTimeout(() => inputRef.current?.focus(), 100) }}
                className="text-[#555555] hover:text-[#f0f0f0] p-1"
              >
                <X size={18} />
              </button>
            </div>
            ...
            <div className="bg-[#1a1a1a] rounded-[16px] p-4 space-y-3">
              <p className="text-xs text-[#888888] font-medium">섭취량</p>
              <div className="flex items-center gap-2">
                <div className="flex-1 bg-[#242424] border border-[#2a2a2a] rounded-[12px] flex items-center px-4 focus-within:border-[#C8FF00]">
                  <input
                    type="number"
                    min={1}
                    value={amountG || ''}
                    ...
                    inputMode="decimal"
                    className="flex-1 bg-transparent py-3 text-lg font-bold text-[#f0f0f0] outline-none tabular-nums text-right"
                  />
                  <span className="text-sm text-[#555555] ml-1.5">g</span>
                </div>
              </div>
              {/* 빠른 선택 */}
                ...
                {servingOptions.map(g => (
                  <button
                    ...
                    className={`flex-1 py-2.5 rounded-[10px] text-xs font-semibold transition-colors ${
                      amountG === g ? 'bg-[#C8FF00] text-[#0f0f0f]' : 'bg-[#242424] text-[#888888]'
                    }`}
                  >
                    {g}g
                  </button>
            ...
            <div className="bg-[#1a1a1a] rounded-[16px] p-4">
              <p className="text-xs text-[#888888] mb-3">{amountG}g 섭취 시 영양소</p>
              <div className="grid grid-cols-4 gap-2">
                {[
                  { label: '칼로리', value: computed.cal, unit: 'kcal', color: '#C8FF00' },
            ...
            {(manualOpen || selected.per100.cal === 0) && (
              <div className="bg-[#1a1a1a] rounded-[16px] p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <p className="text-xs text-[#888888] font-medium">100g 기준 영양소 수정</p>
                  <button onClick={() => setManualOpen(false)} className="text-xs text-[#555555]">접기</button>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  {([
                    { key: 'cal', label: '칼로리', unit: 'kcal', color: '#C8FF00' },
                    ...
                    <div key={key} className="bg-[#242424] rounded-[10px] p-3">
                      ...
                      <div className="flex items-center mt-1.5 gap-1">
                        <input
                          ...
                          inputMode="decimal"
                          placeholder="0"
                          className="flex-1 bg-transparent text-sm text-[#f0f0f0] outline-none tabular-nums"
                        />
            ...
            {!manualOpen && selected.per100.cal > 0 && (
              <button
                ...
                className="text-xs text-[#555555] hover:text-[#888888] underline"
              >
                영양소 직접 수정
              </button>
            )}
      ...
      {/* ── 저장 버튼 (음식 선택 후에만) ── */}
      {selected && (
        <div className="fixed bottom-0 left-0 right-0 px-4 pb-6 pt-3 bg-gradient-to-t from-[#0f0f0f] via-[#0f0f0f]/95 to-transparent z-40">
          <button
            onClick={handleSave}
            ...
            className={`w-full py-4 rounded-[16px] font-bold text-sm transition-all active:scale-[0.98] flex items-center justify-center gap-2 ${
              saved ? 'bg-[#00D67C] text-[#0f0f0f]'
              : canSave ? 'bg-[#C8FF00] text-[#0f0f0f]'
